...
def generate_test_description():
    ...
    rviz2_config =  os.path.join(ament_index_python.get_package_prefix('realsense2lidar'),
            'lib/realsense2lidar/',
            'realsense2lidar.rviz')
    #rviz2_config = os.path.join(ament_index_python.get_package_share_directory('realsense2lidar'),'realsense2lidar', 'realsense2lidar_rviz2.rviz')
    
    # dut -> device under test is the node to be tested in this example
    dut = Node(
        package='realsense2lidar',
        executable='realsense2lidar_node',
        name='realsense2lidar_node',
        parameters= [
            {'frame_id': 'lidar'},
            {'topic_pointcloud_in': 'realsense2lidar/point_cloud_out'},
            {'topic_pointcloud_out': 'realsense2lidar/point_cloud_realsense2lidar'},
            {'leaf_x': 0.2},
            {'leaf_y': 0.2},
            {'leaf_z': 0.2}],
    )
...
class TestProcessOutput(unittest.TestCase):
    ...
    def timer_callback(self):
        ...
        input_data_path = os.path.join(
            ament_index_python.get_package_prefix('realsense2lidar'),
            'lib/realsense2lidar/',
    ...
    def test_dut_output(self, dut, proc_output):
        ...
        sub = self.node.create_subscription(
            PointCloud2,
            '/realsense2lidar/point_cloud_realsense2lidar',
            lambda msg: msgs_rx.append(msg),
            10
        )

        # Publish data to dut
        self.publisher_ = self.node.create_publisher(PointCloud2, 'realsense2lidar/point_cloud_out', 10)
